...
void Mindflayer::addAttack(const int &attack){
    ...
    if(attack == 1){
        ...
        for(int i=0;i < projectiles_.size(); i++){// check if proj pisonic is there. if there set bool proj to true and stop running 
    ...
    if(attack == 2){
        bool proj = false;// rest of the implmenation are the same as the previous one. But checking for different Variant 
        for(int i=0;i < projectiles_.size(); i++){
            if(projectiles_[i].type_ == TELEPATHIC){
                proj = true;
                break;
