...
*/
    Mindflayer(const std::string &new_name, Category new_type = ALIEN, int new_hitpoint = 1,int new_level =1 , bool new_tame = false, 
std::vector<Projectile> new_projectile = {}, bool new_summon = false, std::vector<Variant> new_variant = {});
...
*/
    void setProjectiles(const std::vector<Projectile> &new_projectile);
...
*/
    void setSummoning(const bool &new_summon);
...
*/
    void setAffinities(const std::vector<Variant> &new_variant);

/**
  @param       : A reference to the Variant 
  @return      : The string representation of the variant
*/
    std::string variantToString(const Variant &new_variant);
