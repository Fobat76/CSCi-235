...
*/
bool Creature::operator== (const Creature &reference)const{
    if(reference.name_ == name_ && reference.level_ == level_ && reference.category_ == category_ && reference.tame_ == tame_){
        return true;
    }
    return false;
}
